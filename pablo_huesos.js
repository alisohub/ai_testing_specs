...
// ==========================================
    ...
    Main.init = function() {
        ...
        // Индексы 0-3 для дневной смены, индексы 4-7 для ночной. Перерыв №4 это индексы 3 и 7.
        const isNight = store.sessionConfig.shiftType === 'night';
        store.sessionConfig.selectedLunchIndex = isNight ? 7 : 3;

        // 3. ЕДИНЫЙ СТИЛЬ ТЕКСТА ДЛЯ ВСЕХ ВКЛАДОК
        const unifiedColor = '#808080'; // Белый цвет
        const unifiedAlpha = 40;        // Прозрачность 40%
        const unifiedFontSize = 11;     // Размер текста 11px (строго число, движок сам добавит 'px')

        // Массив ключей всех строк для автоматической итерации
        const lines = [
        ...
        // Применяем настройки ко всем строкам в цикле (DRY паттерн)
        lines.forEach(lineKey => {
            store.localTabConfig.linesConfig[lineKey].visible = true;
            store.localTabConfig.linesConfig[lineKey].colorHex = unifiedColor;
            store.localTabConfig.linesConfig[lineKey].alpha = unifiedAlpha;
            store.localTabConfig.linesConfig[lineKey].fontSize = unifiedFontSize;
        });

        // Отключаем мультиколор для второй строки (принудительно делаем её монотонной)
        store.localTabConfig.linesConfig.line2_globalSummary.multicolor = false;

        // Глобальные галочки: считать статистику со всех отделов в общую сумму
        ...
        // 4. Фиксация состояния (сохранение в localStorage и триггер перерисовки DOM/CSS)
        StorageManager.saveState();
    };
